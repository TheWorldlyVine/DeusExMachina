...
module "auth_function" {
  ...
  env_vars = merge(
    ...
    var.use_cloud_sql ? {
      CLOUD_SQL_CONNECTION_NAME = "${var.project_id}:${var.region}:${google_sql_database_instance.auth_sessions[0].name}"
    } : {},
    var.use_redis ? {
      REDIS_HOST = google_redis_instance.auth_cache[0].host
      REDIS_PORT = google_redis_instance.auth_cache[0].port
    } : {}
  )
